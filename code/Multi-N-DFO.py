...
open('Multi-N-Fitness.txt', 'w').close()  # Clearing the textfile on every run
...
lowerB = [-5] * D  # LOWER BOUND (IN ALL DIMENSIONS)
upperB = [5] * D  # UPPER BOUND (IN ALL DIMENSIONS)
...
def multi():
    ...
    with open("Multi-N-Fitness.txt", "a+") as f:
        f.write(f"{fitness[s]}\n")  # :.50f}\n")
        f.close()
...
if __name__ == "__main__":
    ...
    for process in processes:
        process.join()
    tEnd = perf_counter()
    newLis = np.loadtxt('Multi-N-Fitness.txt', delimiter="\n")
